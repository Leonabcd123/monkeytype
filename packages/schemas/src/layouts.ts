...
export let LayoutNameSchema = z.enum(layoutNames as [string, ...string[]], {
  errorMap: customEnumErrorHandler("Must be a supported layout"),
});

export type LayoutName = z.infer<typeof LayoutNameSchema>;

export function updateLayouts(layoutName: string): void {
  layoutNames.push(layoutName);

  LayoutNameSchema = z.enum(layoutNames as [string, ...string[]], {
    errorMap: customEnumErrorHandler("Must be a supported layout"),
  });
}
