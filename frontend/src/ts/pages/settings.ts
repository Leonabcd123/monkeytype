...
ConfigEvent.subscribe((eventKey, eventValue) => {
  ...
  if (eventKey === "themeLight") {
    $(
      `.pageSettings .section[data-config-name='autoSwitchThemeInputs'] select.light option[value="(${Strings.safeToString(
        eventValue
      )}"]`
    ).attr("selected", "true");
  } else if (eventKey === "themeDark") {
    $(
      `.pageSettings .section[data-config-name='autoSwitchThemeInputs'] select.dark option[value="${Strings.safeToString(
        eventValue
      )}"]`
    ).attr("selected", "true");
  }
